...
type Identity struct {
	...
	// adapters. Typical keys are instructions, project, task, memory, and
	// source:<name>. They are freshness inputs, not cache-key inputs.
...
type State struct {
	...
	InstructionRecords []Instruction                     `json:"instruction_records,omitempty"`
	InstructionExplain map[string]InstructionExplanation `json:"instruction_explain,omitempty"`
...
type Freshness struct {
	...
	InstructionVersion uint64            `json:"instruction_version,omitempty"`
	ComponentVersions  map[string]string `json:"component_versions,omitempty"`
...
func Resume(root string, id Identity, budget int) (ResumeResult, error) {
	...
	if errors.Is(err, os.ErrNotExist) {
		...
		if err := reconcileInstructionState(&r.Snapshot, id); err != nil {
	...
	previousInstructionVersion := s.Freshness.InstructionVersion
	if err := reconcileInstructionState(&s, id); err != nil {
		return ResumeResult{}, err
	}
	if previousInstructionVersion != s.Freshness.InstructionVersion {
		// The registry was read locally and differs from the persisted snapshot.
		changed = append(changed, "component:instructions")
		status = "stale"
	}
...
func checkpointStateLocked(root string, id Identity, state State, materialize bool) (Snapshot, error) {
	var err error
	records := state.InstructionRecords
	if len(records) == 0 {
		for _, rule := range state.Instructions {
			if rule.RegistrySource == "" {
				records = append(records, rule)
			}
		}
	}
	state.InstructionRecords, err = NormalizeCheckpointInstructions(records, id)
	if err != nil {
		return Snapshot{}, err
	}
	state.Instructions = append([]Instruction(nil), state.InstructionRecords...)
	state.InstructionExplain = nil
	if materialize {
...
func RefreshDetailed(root string, id Identity) (RefreshResult, error) {
	...
	if previousInstructionVersion != s.Freshness.InstructionVersion {
		refreshed = unique(append(refreshed, "instructions"))
	}
...
func reconcileInstructionState(snapshot *Snapshot, id Identity) error {
	registry, err := MaterializeInstructions(id)
	if err != nil {
		return err
	}
	records := snapshot.State.InstructionRecords
	if len(records) == 0 {
		for _, rule := range snapshot.State.Instructions {
			if rule.RegistrySource == "" {
				records = append(records, rule)
			}
		}
	}
	resolved, err := ReconcileInstructions(records, registry, id, time.Now().UTC())
	if err != nil {
		return err
	}
	// Registry materialization replaces only registry-owned instruction
	// metadata. Retaining an old required gap would claim a revoked or repaired
	// rule still applies.
	snapshot.State.Gaps = withoutInstructionGaps(snapshot.State.Gaps)
	snapshot.State.Conflicts = withoutInstructionConflicts(snapshot.State.Conflicts)
	snapshot.State.Instructions = resolved.Instructions
	snapshot.State.InstructionRecords = records
	snapshot.State.InstructionExplain = resolved.Explain
	snapshot.State.Gaps = append(snapshot.State.Gaps, resolved.Gaps...)
	snapshot.State.Conflicts = append(snapshot.State.Conflicts, resolved.Conflicts...)
	snapshot.Freshness.InstructionVersion = resolved.Version
	return nil
}
func withoutInstructionGaps(gaps []Gap) []Gap {
	out := gaps[:0]
	for _, gap := range gaps {
		if gap.Source != "deja://instructions" {
			out = append(out, gap)
		}
	}
	return out
}
func withoutInstructionConflicts(conflicts []Conflict) []Conflict {
	out := conflicts[:0]
	for _, conflict := range conflicts {
		if conflict.Source != "deja://instructions" {
...
func Inspect(root string, id Identity) (Status, error) {
	...
	if previousInstructionVersion != s.Freshness.InstructionVersion {
		c = append(c, "component:instructions")
	}
	sourceAvailabilityGaps(&s.State)
...
func validateSourceState(layer string, state State) error {
	...
	case "memory":
		if state.Objective != "" || state.Status != "" || state.Project != nil || state.ProjectSource != "" || state.TestsSource != "" || len(state.Instructions)+len(state.InstructionRecords)+len(state.InstructionExplain)+len(state.ProjectItems)+len(state.Confirmed)+len(state.Implemented)+len(state.Failing)+len(state.Unknown)+len(state.Decisions)+len(state.NextActions)+len(state.Session)+len(state.Gaps)+len(state.Conflicts)+len(state.Promoted) != 0 || state.Tests != nil {
...
func stateIsOnlyProject(s State) bool {
	return s.Objective == "" && s.Status == "" && s.ProjectSource == "" && s.TestsSource == "" && len(s.Instructions)+len(s.InstructionRecords)+len(s.InstructionExplain)+len(s.Confirmed)+len(s.Implemented)+len(s.Failing)+len(s.Unknown)+len(s.Decisions)+len(s.NextActions)+len(s.Session)+len(s.Evidence)+len(s.Gaps)+len(s.Conflicts)+len(s.Promoted) == 0 && s.Tests == nil
}
func stateIsOnlyTask(s State) bool {
	return s.Project == nil && s.ProjectSource == "" && s.TestsSource == "" && len(s.ProjectItems)+len(s.Instructions)+len(s.InstructionRecords)+len(s.InstructionExplain)+len(s.Session)+len(s.Evidence)+len(s.Promoted) == 0
}
...
func flatten(s State) map[string]string {
	...
	for _, instruction := range s.Instructions {
		...
		m["instructions/"+instruction.ID] = string(b)
	}
	sets := map[string][]Item{"project": s.ProjectItems, "confirmed": s.Confirmed, "implemented": s.Implemented, "failing": s.Failing, "unknown": s.Unknown, "decisions": s.Decisions, "next_actions": s.NextActions, "session": s.Session, "evidence": s.Evidence}
...
func Explain(root string, id Identity, itemID string) (Explanation, error) {
	...
	if !ok {
		if explanation, ok := s.State.InstructionExplain[itemID]; ok {
			instruction := explanation.Instruction
			why := explanation.Disposition + ": " + explanation.ResolutionReason
			if stale {
				why += "; snapshot is stale"
			}
			return Explanation{Instruction: &instruction, Section: "instructions", SnapshotID: s.ID, Identity: s.Identity, Freshness: s.Freshness, InvalidLayers: append([]string(nil), s.Invalid...), Why: why}, nil
		}
		for i := range s.State.Instructions {
			instruction := &s.State.Instructions[i]
			if instruction.ID == itemID {
				why := "applicable instruction resolved for the current workspace and task; source " + instruction.Source
				if stale {
					why += "; snapshot is stale"
				}
				return Explanation{Instruction: instruction, Section: "instructions", SnapshotID: s.ID, Identity: s.Identity, Freshness: s.Freshness, InvalidLayers: append([]string(nil), s.Invalid...), Why: why}, nil
...
func trimResult(r *ResumeResult, budget int) (bool, error) {
	...
	r.Truncated = true
	s := &r.Snapshot.State
	// Audit records can be retrieved through ctx explain. Remove them before
	// active context, and discard optional advice before any binding rule.
	s.InstructionRecords = nil
	s.InstructionExplain = nil
	for i := len(s.Instructions) - 1; i >= 0; i-- {
		if s.Instructions[i].Priority == "preferred" || s.Instructions[i].Priority == "advisory" {
