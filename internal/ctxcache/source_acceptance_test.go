...
func sourceAcceptanceMissingInstructionRegistry(t *testing.T) {
	t.Helper()
	t.Setenv("DEJA_INSTRUCTIONS_FILE", filepath.Join(t.TempDir(), "instructions.json"))
}
...
func TestSourceAcceptanceRefreshPreservesOtherLayersAndProvenance(t *testing.T) {
	sourceAcceptanceMissingInstructionRegistry(t)
	root := t.TempDir()
	dir := t.TempDir()
