...
func isolated(t *testing.T) string {
	t.Helper()
	d := t.TempDir()
	// macOS commonly exposes the temporary root through /var, while filesystem
	// resolution returns the physical /private/var path. Fixtures that exercise
	// path equality must use the same canonical spelling as the resolver.
	var err error
...
func TestExplicitSupersessionAndEffectiveWindow(t *testing.T) {
	...
	if err != nil || len(result.Conflicts) != 1 || result.Conflicts[0].Key != "supersession:old" {
		t.Fatalf("competing successors were not explicit: %#v %v", result, err)
	}
