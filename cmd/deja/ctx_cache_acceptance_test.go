...
func ctxFixtureEnvironment(t *testing.T) (workspace, indexDir string) {
	...
	if err := os.WriteFile(filepath.Join(root, "instructions.json"), ctxFixture(t, "approved-registry.json"), 0600); err != nil {
		t.Fatal(err)
	}
...
func TestCtxCachePublicBudgetPreservesRequiredInstructions(t *testing.T) {
	workspace, dir := ctxFixtureEnvironment(t)
	state := ctxcache.State{Objective: "Preserve required context", Session: []ctxcache.Item{{ID: "scratch", Text: strings.Repeat("temporary detail ", 2000), Source: "file://synthetic/session"}}}
	...
	for name, raw := range map[string][]byte{"CLI": cli, "MCP": []byte(mcp)} {
		...
		if !strings.Contains(string(raw), "INS-telemetry") || !strings.Contains(string(raw), "Validate telemetry before closing this task.") {
			t.Fatalf("%s omitted required instruction: %s", name, raw)
		}
	}
	if string(cli) != mcp {
...
func TestCtxCacheLocalSourceFixtureRefreshesOnlyTask(t *testing.T) {
	...
	explanation := ctxFixtureCLI(t, dir, workspace, "explain", nil, "--item", "INS-telemetry")
	if !strings.Contains(string(explanation), "file://synthetic/acceptance.md#telemetry") {
		t.Fatalf("instruction explain missing provenance: %s", explanation)
	}
