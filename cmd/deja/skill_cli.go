...
const cliSkillBody = `At the beginning of substantive work, run ` + "`deja ctx resume --workspace \"$PWD\"`" + ` and include ` + "`--task <active-task-id>`" + ` when known. Treat applicable ` + "`absolute`" + ` and ` + "`required`" + ` instructions in the returned context as binding. Use that packet before independently reconstructing project history.
